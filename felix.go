...
func handlePostRequest(client *mongo.Client) {
	...
	startTime := time.Date(pastFiveDays.Year(), pastFiveDays.Month(), pastFiveDays.Day(), 0, 0, 0, 0, pastFiveDays.Location())
	...
	initialTime := uint64(startTimeMillis)
	processTillTime := uint64(endTimeMillis)

	for _, v := range batDataAllObjArray {

		if v.LASTTIME != 0 && v.LASTTIME < initialTime {

			filter1 := bson.M{"timestamp": bson.M{"$gte": initialTime, "$lt": processTillTime}, "bid": v.BID}
			...
			dataToIns.BID = v.BID
			dataToIns.FROM = initialTime
			dataToIns.TO = processTillTime
			...
			toUpdate := bson.M{
				"$set": bson.M{
					"lastProcessedAnalytics": processTillTime,
				},
			}
